...
const AcoountButtonHeader = ({ user }: { user: UserType }) => {
    ...
    return (
        <>
            {showDrop ? (<span className='fixed w-screen left-0 top-0 h-screen  block z-[20]' onClick={() => setShowDrop(false)}></span>) : ""}
            <div onClick={() => setShowDrop(!showDrop)} className='relative z-[20] flex justify-between items-center  p-1 h-[3.125rem] bg-white gap-2  font-bold rounded-3xl text-center max-[15.625rem] cursor-pointer'>
